...
Данный файл содержит две функции: ```is_photo_small()``` и ```get_names_photos()```. 

Данные скрипты не являются самостоятельными. При запуске через консоль, скрипты не выполнят своих функций. 

Данные скрипты является вспомогательными. Функция ```is_photo_small()``` используется в функции  ```get_names_photos()```, а она, в свою очередь,  в файлах ```download_to_telegram.py``` и ```infinity_download_to_telegram.py```.

# is_photo_small()

Данная функция принимает на вход путь фотографии. 

Затем происходит проверка размера файла. Это необходимо для определения возможности отправления данной фотографии в телеграм. 

Если файл превышает размер в 20 Мб, то такую фотографию отправить невозможно и функция вернет ```False```

Если файл меньше 20 Мб, то такую фотографию в телеграм можно отправить, соответственно, функция вернет ```True```

# get_names_photos()

Данная функция предназначена для создания списка файлов содержащихся в папке image. Полученный список является выходным параметром функции. 

Внутри функции также происходит проверка пригодности файлов для отправления в телеграм с помощью ```is_photo_small()```.

Файлы, размер которых превышает 20 Мб, не попадут в итоговый список файлов. 
